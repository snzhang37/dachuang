...
class ServiceRecovery():
            ...
            for src in src_domain:
                ...
                for snk in snk_domain:
                    ...
                    try:  # 用内置BFS函数，速度快，但成功率会低，不知道为啥
                        ...
                        # all_paths = nx.dijkstra_path(self.G_sub, src, snk)
                        all_paths = list(
                            islice(nx.shortest_simple_paths(self.G_sub, src, snk), num_route))  # 这个参数对结果也有很大的影响
                        # all_paths = list(islice(nx.all_simple_paths(self.G_sub, src, snk), num_route))  # 这个参数对结果也有很大的影响，不可行，这个使用的是DFS，BFS更高效
                        # all_paths = self.bidirectional_bfs_with_constraints(src, snk, src_rest, snk_rest, 4)  # 最后一个参数是最大深度，可调
    ...
    # 域内路由逻辑封装
    def _handle_domain_routing(self, subgraph_index, s, d, sub_routes, num_route = 2):
        if self.subgraphs[subgraph_index].graph['is_allrelay'] == True:
            ...
            # route = list(islice(nx.shortest_simple_paths(self.subgraphs[subgraph_index], s, d), num_route))
            # if len(route) > 0:
            #     return  0, sub_routes
            #
            # route = route[0]
    ...
    def updata_spectrum(self, sub, path, l):
        for i in range(len(path) - 1):
            self.G[path[i]][path[i + 1]]['f_slot'].remove(l)
            try:
                sub[path[i]][path[i + 1]][0]['f_slot'].remove(l)
            except KeyError:
                sub[path[i]][path[i + 1]]['f_slot'].remove(l)
    ...
    def updata_state(self, domain_route, sub_routes):
        ...
        for index_r, r in enumerate(sub_routes):
            ...
            route = r['route'][0]
            layer = r['layer']
            self.updata_spectrum(self.subgraphs[r['domain_index']], route, layer)

            current_shared_layers = self.subgraphs[r['domain_index']].graph['shared_layers'] # 更新空余共享波长
            if layer in current_shared_layers:
                current_shared_layers.remove(layer)
            self.subgraphs[r['domain_index']].graph['shared_layers'] = current_shared_layers
    ...
    def defragment_graph(self, G):
        """
        如果整个图G中所有边没有共同的可用波长或者不是连通图，则需要拆分图，
        拆分的结果要求每个子图内的所有边都有至少一个共同可用波长，且每个子图是连通图。

        返回：一个子图列表，每个子图为 G 的一个连通子图，且该子图内所有边的 f_slot 交集非空。
        """
        # 获取图中所有边（无向图中 (u,v) 和 (v,u) 只保留一次）
        all_edges = list(G.edges())
        partitions = []  # 每个元素为一个集合，存储边的元组

        for e in all_edges:
            u, v = e
            S_e = set(self.G[u][v]['f_slot'])
            assigned = False

            # 遍历已有分区
            for part in partitions:
                # 计算当前分区内所有边的 f_slot 交集
                common = None
                for pe in part:
                    pu, pv = pe
                    pe_slots = set(self.G[pu][pv]['f_slot'])
                    if common is None:
                        common = pe_slots
                    else:
                        common &= pe_slots
        ...
        # 根据 partitions 构造子图，并进一步拆分成连通分量
        connected_components = []
        for part in partitions:
            # 1 构造包含分区所有边的图 H
            H = nx.Graph()
            H.add_edges_from(part)

            # 2 找到所有连通子图
            for component in nx.connected_components(H):
                # 3. 为该连通子图创建子图
                connected_subgraph = H.subgraph(component).copy()
                connected_components.append(list(connected_subgraph.edges))
    ...
    def update_subgraph(self, domain_route): # 更新域级图，对于本次路由涉及到的各个域，检查每个域是否是连通图，这是作为域的基本条件，不满足的话需要将这个域重新聚类，并调整和邻域的关系
        defragment_flag = False # 标志有无进行域的拆分

        new_subs = [] # 每个元素是一个字典，键是子图索引，键值是其拆分形成的子图
        for i in domain_route:
            connected_components = []
            if not nx.is_connected(self.subgraphs[i]) or len(self.subgraphs[i].graph['shared_layers']) == 0: # 不是连通图或者没有共享空闲波长
                self.subgraphs[i].graph['valid'] = False # 这个子图即将被拆分或丢弃，后续不再使用这个子图本身，为了减小G_sub的更新复杂度，这里不将其删除，只是将其屏蔽
                connected_components = self.defragment_graph(self.subgraphs[i])
            ...
            if len(connected_components) > 0:
                ...
                for component in connected_components:
                    sub_ori = self.subgraphs[i].copy()
                    try:
                        new_subg = sub_ori.edge_subgraph(component) # 默认是冻结图
                        new_subg = new_subg.copy() # 创建副本，解除冻结
                        new_sub.append(new_subg)
                    except ValueError:
                        # 在每个二元组后添加 0
                        component = [(x, y, 0)  for x, y in component]

                        new_subg = sub_ori.edge_subgraph(component)  # 默认是冻结图
                        new_subg = new_subg.copy()  # 创建副本，解除冻结
                        new_sub.append(new_subg)

                # 添加margin_nodes和is_allrelay和shared_layers属性
                for new_s in new_sub:
                    ...
                    slot_sets = [set(data.get('f_slot', [])) for _, _, data in new_s.edges(data=True)]
                    shared_slot = list(set.intersection(*slot_sets)) if slot_sets else []

                    new_s.graph['shared_layers'] = shared_slot
    ...
    def chosen_route(self, sub_routes):
        ...
        if len(slices) == 0: # 整条路径没有使用电中继，共享同一个可用频隙列表
            ...
            # 提取出沿途的各段的共享空闲波长
            shared_layers = []
            domain_indexes = [item["domain_index"] for item in sub_routes]
            shared_layers += [item for i in domain_indexes for item in self.subgraphs[i].graph['shared_layers']]
            shared_layers = list(set(shared_layers))

            suggest_layers = list(set(layers) - set(shared_layers))

            if len(suggest_layers) == 0:
                chosen_layer = min(layers)
            else:
                chosen_layer = min(suggest_layers)

            for sr in sub_routes:
                sr['layer'] = chosen_layer
            ...
            for j in range(len(slices)+1): # 如果slices中有m个元素，则总路径被划分为m+1段
                ...
                # 提取出沿途的各段的共享空闲波长
                shared_layers = []
                domain_indexes = [item["domain_index"] for item in paths]
                shared_layers += [item for i in domain_indexes for item in self.subgraphs[i].graph['shared_layers']]
                shared_layers = list(set(shared_layers))

                suggest_layers = list(set(common_layer) - set(shared_layers))
    ...
    def run(self):
        ...
        for index, service in enumerate(self.services):
            ...
            # # 由于先处理绝对距离较远的业务，相当于先处理阻塞概率大的业务，所以这里是根据业务的序号来决定域间路径的个数。反比关系 num_routes = [3,2,2,1,1] # 域间路径计算可用数目
            # num_route = math.floor(len(num_routes) * (index + (len(self.services))/len(num_routes)) / len(
            # self.services)) domain_routes = self.domain_route(service, num_routes[num_route-1])  # 查找域间路径

            domain_routes = self.domain_route(service, 5)  # 查找域间路径
            ...
            # domain_routes_flat = sorted(domain_routes_flat, key=len, reverse=True)
                ...
                while len(domain_routes_flat) > 0:
                    ...
                    if success_flag1: # 业务路由成功
                        ...
                        if real_success_flag:
                            ...
                            start_update = time.time()
                            defragment_flag = self.update_subgraph(domain_route)  # 更新域级图，对于本次路由涉及到的各个域，检查每个域是否是连通图，这是作为域的基本条件，不满足的话需要将这个域重新聚类，并调整和邻域的关系
                            update_times += defragment_flag
                            end_update = time.time()
                            time_for_update.append(end_update - start_update)
...
if __name__ == '__main__':
    ...
    # band = 24
    # c_max = 964
    # subgraph_file_path = './subgraphs/'+FILE_ADD+'example1_1/'
    ...
    # band = 8
    # c_max = 864
    # subgraph_file_path = './subgraphs/'+FILE_ADD+'example2_2/'
    ...
    band = 8
    c_max = 968
    subgraph_file_path = './subgraphs/' + FILE_ADD + 'example6_6/'
    ...
    # band = 24
    # c_max = 984
    # subgraph_file_path = './subgraphs/' + FILE_ADD + 'example5_5/'
