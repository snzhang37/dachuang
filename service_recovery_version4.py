...
class ServiceRecovery():
    ...
    def chosen_route(self, sub_routes):
        ...
        if len(slices) == 0: # 整条路径没有使用电中继，共享同一个可用频隙列表
            ...
            # chosen_index = layers.index(max(layers))
            layer = random.sample(layers,1)
            chosen_index = layers.index(layer[0])
            ...
            for j in range(len(slices)+1): # 如果slices中有m个元素，则总路径被划分为m+1段
                ...
                # chosen_layer = max(common_layer)
                chosen_layer = random.sample(common_layer,1)
                chosen_layer = chosen_layer[0]
    ...
    def run(self):
        ...
        for index, service in enumerate(self.services):
            ...
            domain_routes = self.domain_route(service, 1)  # 查找域间路径
