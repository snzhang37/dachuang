...
class ServiceRecovery():
    def __init__(self, distance_margin, ots_margin, osnr_margin, subgraph_file_path, file_path='example/', file_name='example1', band=24,c_max = 964, FILE_ADD = 'RSOP_50/', **kwargs):
        ...
        self.ori_ave_degree =  sum(dict(self.G.degree()).values()) / self.G.number_of_nodes()
        self.subgraphs = load_sg(subgraph_file_path) # 载入聚类结果

        self.G_sub = self.G_domain() # 将域抽象为节点的子图，体现域间连接
        self.domain_ave_degree = sum(dict(self.G_sub.degree()).values()) / self.G_sub.number_of_nodes()
    ...
    def get_relay_node(self):
        filename = self.file_path + self.FILE_ADD + self.file_name + '.relay.csv'
        ...
        for i in domain_route:
            if not nx.is_connected(self.subgraphs[i]): # 不是连通图
                ...
                if all(len(s) == 1 for s in connected_components):
                    ...
                    #
                    #     new_subg = sub_ori.subgraph(component)  # 默认是冻结图
    ...
    def verify(self, route):
        for r in route:
            ...
            for j in range(len(path) - 1):
                if l not in self.G[path[j]][path[j + 1]]['f_slot']:
                    raise ValueError
    ...
    def run(self):
     ...
     #   print(resource_occupation_before)
        ...
        for index, service in enumerate(self.services):
            ...
            domain_routes = self.domain_route(service, 3)  # 查找域间路径
            ...
            if len(domain_routes_flat) > 0:
                shortest_path_len.append(sum(len(sublist) for sublist in domain_routes_flat)/len(domain_routes_flat))
                ...
                while len(domain_routes_flat) > 0:
                    ...
                    if success_flag1: # 业务路由成功
                        ...
                        if real_success_flag:
                            ...
                            start_update = time.time()
                            defragment_flag = self.update_subgraph(domain_route)
                            update_times += defragment_flag
                              # 更新域级图，对于本次路由涉及到的各个域，检查每个域是否是连通图，这是作为域的基本条件，不满足的话需要将这个域重新聚类，并调整和邻域的关系
                            end_update = time.time()
        ...
        END = time.time()
        return num_succeed, len(self.services), (END - START)/len(self.services), self.resource_occupation(self.G)
