...
def cluster(G, distance_margin, ots_margin, osnr_margin, file_name, FILE_ADD,**kwargs): # ex用于指示是对第几个例子中的拓扑进行聚类
    ...
    while len(all_nodes) != 0:
        ...
        if G.nodes[start_node]['relay'] == False:# 只对非中继节点进行聚类
            ...
            new_subgraph = nx.MultiGraph() # 无向多重边图
        ...
        if all(not set(new_subgraph.edges()).intersection(set(sg.edges())) for sg in subgraphs) and not nx.is_empty(
           ...
           # print(num)
            ...
            count_edges += len(list(new_subgraph.edges))
    ...
    while count_edges != len(list(G.edges)):
        for i in G.nodes():
            if G.nodes[i]['degree'] < G.degree(i):
                if G.nodes[i]['relay'] == True:
                    ...
                    for n2 in G.neighbors(i):
                        ...
                        path_distance = max(G.get_edge_data(i, n2)[j]['distance'] for j in
                                             range(len(G.get_edge_data(i, n2))))
                        path_ots = max(G.get_edge_data(i, n2)[j]['ots'] for j in
                                            range(len(G.get_edge_data(i, n2))))
                        path_osnr = max(G.get_edge_data(i, n2)[j]['osnr'] for j in
                                            range(len(G.get_edge_data(i, n2))))
                    ...
                    if not nx.is_empty(new_subgraph):
                       ...
                       # print(count_single)
                        count_edges += len(list(new_subgraph.edges))
                       # print(f"当前边数:{count_edges} / {len(list(G.edges))}")
   ...
   # print(f"单个节点成子图的个数={count_single}")
    ...
    # 保存子图
    if not os.path.exists('subgraphs1/' + FILE_ADD + file_name):
        os.makedirs('subgraphs1/' + FILE_ADD + file_name)
    for i in range(len(subgraphs)):
        save_pkl(path + FILE_ADD + file_name + '/' + 'sg' + str(i) + '.pkl', subgraphs[i])
...
if __name__ == '__main__':
    ...
    file_name = 'example_200' # 0.6是一个普适参数 # 均匀/不均匀大规模 5/4
    file_name_sub = 'example_200'
    G = create_topology(file_path='new_example/' + FILE_ADD, file_name=file_name, band=8, c_max=968)
